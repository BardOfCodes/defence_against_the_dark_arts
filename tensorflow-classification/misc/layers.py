...
def res_block_dif(bottom, name, weights, stride=1, first=False):
    layers = {}
    with tf.name_scope(name+'_a'):
        layers['conv_1'] = conv_layer(bottom['final'], weights['res'+name+'_branch2a']['weights'], s=stride, relu=False)
        layers['bn_1'] = batch_norm(layers['conv_1'], weights['bn'+name+'_branch2a'])

    with tf.name_scope(name+'_b'):
        layers['conv_2'] = conv_layer(layers['bn_1'], weights['res'+name+'_branch2b']['weights'], relu=False)
        layers['bn_2'] =  batch_norm(layers['conv_2'], weights['bn'+name+'_branch2b'])

    with tf.name_scope(name+'_c'):
        layers['conv_3'] = conv_layer(layers['bn_2'], weights['res'+name+'_branch2c']['weights'], relu=False)
        layers['bn_3'] = batch_norm(layers['conv_3'], weights['bn'+name+'_branch2c'], relu=False)

    if first:
        with tf.name_scope(name+'_1'):
            layers['conv_4'] = conv_layer(bottom['final'], weights['res'+name+'_branch1']['weights'], s=stride, relu=False)
            layers['bn_4'] = batch_norm(layers['conv_4'], weights['bn'+name+'_branch1'], relu=False)
        layers['final'] = tf.nn.relu(tf.add(layers['bn_4'],layers['bn_3']))
        return layers
    else:
        layers['final'] =tf.nn.relu(tf.add(bottom['final'],layers['bn_3']))
        return layers
